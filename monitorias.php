    ...
    <div class="container">
        ...
        <div class="projects-grid">
            ...
            if ($resultado->num_rows > 0) {
                while($monitoria = $resultado->fetch_assoc()) {
                    ...
                    $viewUrl = 'monitor.php?id=' . (int) $monitoria['idMonitoria'];
                    
                    echo '<div class="project-card tipo-' . htmlspecialchars($monitoria['tipoMonitoria']) . '" data-id="' . (int) $monitoria['idMonitoria'] . '" data-tipo="' . htmlspecialchars($monitoria['tipoMonitoria']) . '" data-view-url="' . htmlspecialchars($viewUrl) . '">';
                    echo '<img src="' . htmlspecialchars($imagemCapa) . '" alt="' . htmlspecialchars($monitoria['nome']) . '" class="project-image" onerror="this.onerror=null;this.src=\'assets/photos/default-monitoria.jpg\';">';
                    echo '<div class="project-label ' . $corClass . '">' . htmlspecialchars($nomeExibido) . '</div>';
                    echo '</div>';
                }
            } else {
                // Caso não tenha monitorias cadastradas, mostrar mensagem
