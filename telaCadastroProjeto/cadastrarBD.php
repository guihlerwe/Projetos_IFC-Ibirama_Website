...
$txtLinkBolsista = $_POST["link-bolsista"] ?? '';
$email = $_POST["email"] ?? '';
...
$stmt = $conn->prepare("
    INSERT INTO projeto 
    (nome, tipo, categoria, anoInicio, linkParaInscricao, textoSobre, linkSite, email, numero, linkInstagram, capa, banner) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
");

$stmt->bind_param(
    "ssssssssssss",
    $nomeProjeto, $tipo, $categoria, $anoInicio, $txtLinkInscricao,
    $txtSobre, $txtLinkSite, $email, $numeroTelefone, $instagram, $nomeCapa, $nomeBanner
);
